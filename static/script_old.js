...
let directionsService, directionsRenderer;
    ...
    function bookAmbulance(location) {
        return new Promise((resolve) => {
            setTimeout(() => {
                resolve({
                    ambulanceLat: location.lat() + (Math.random() - 0.5) * 0.01,
                    ambulanceLng: location.lng() + (Math.random() - 0.5) * 0.01
                });
            }, 1000);
        });
    }
    ...
    function updateAmbulancePosition(ambulancePos, userPos) {
        return new Promise((resolve) => {
            directionsService.route({
                ...
                if (status === 'OK') {
                    ...
                    ambulanceMarker.setPosition(leg.steps[0].end_location);
                    resolve();
                }
            });
        });
    ...
    $(document).ready(function() {
        ...
        $('#ambulance-form').on('submit', function(e) {
            e.preventDefault();
            var name = $('#name').val();
            var phone = $('#phone').val();
            var location = $('#location').val();

            $('#status').text('Booking ambulance for ' + name + '...');

            var userPos = userMarker.getPosition();
            if (ambulanceMarker) ambulanceMarker.setMap(null);

            bookAmbulance(userPos).then((ambulancePos) => {
                ...
                ambulanceMarker = new google.maps.Marker({
                    position: new google.maps.LatLng(ambulancePos.ambulanceLat, ambulancePos.ambulanceLng),
                    map: map,
                    icon: ambulanceIcon
                });

                function moveAmbulance() {
                    updateAmbulancePosition(ambulanceMarker.getPosition(), userPos).then(() => {
                        if (google.maps.geometry.spherical.computeDistanceBetween(ambulanceMarker.getPosition(), userPos) > 50) {
                            setTimeout(moveAmbulance, 1000);
                        } else {
                            $('#status').text('Ambulance has arrived!');
                        }
                    });
                }
